...
object Exercise5 {
  ...
  */
  def sumAllLists(lists: List[List[Double]]): Double = lists.map(sumList).sum

  /*
  Given a List of Integers, return a new List with each element squared
  squaredValues(List(1,2,3)) → List(1,4,9)
   */
  def squaredValues(list: List[Int]): List[Int] = list.map(Math.pow(_, 2).toInt)

  /*
  Given a List of Strings, return a new List with sum of the character "x" in each String
   */
  def xCounts(list: List[String]): List[Int] = list.map(countX)
  ...
  def uniqueValues(list: List[String]): Int = list.distinct.size
 ...
 */
  ...
  def uniqueNames(list: List[Animal]): Int = {
    list
      .filterNot(_.isInstanceOf[Parrot])
      .map(_.name)
      .distinct
      .size
  }
